...
public class AviserSolrJClient extends AbstractSolrJClient{
    private static HttpSolrServer aviserSolrServer;
    ...
    static{ 
        ...
        try{
            ...
            aviserSolrServer = new HttpSolrServer(LicenseModulePropertiesLoader.AVISER_SOLR_SERVER);
            aviserSolrServer.setRequestWriter(new BinaryRequestWriter()); //To avoid http error code 413/414, due to monster URI. (and it is faster)
                
        }
        catch(Exception e){
            System.out.println("Unable to connect to aviser Solr server");
            e.printStackTrace();
            log.error("Unable to connect to aviser Solr server",e);
        }
    }
    ...
    public static ArrayList<String>  filterIds(ArrayList<String> ids, String queryPartAccess) throws Exception{
        ...
        QueryResponse response = aviserSolrServer.query(query);
